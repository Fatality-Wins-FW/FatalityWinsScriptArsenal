...
local function isEnemy(p)
    ...
    if pos.Y < -50 or pos.Y > 400 or (pos - Vector3.new(0,0,0)).Magnitude > 900 then
        return false
    end

    if p.Team ~= LocalPlayer.Team then return true end
    local myTorso = LocalPlayer.Character:FindFirstChild("UpperTorso")
    ...
    pcall(function()
        for _, v in next, ReplicatedStorage.Weapons:GetChildren() do
            ...
            if getgenv().FatalityState.Auto then
                ...
                if m then require(m).Auto = true end
            end
        end
    end)
end)
        ...
        if getgenv().FatalityState.Rage then
            pcall(function()
                ...
                if target then
                    ...
                    hrp.CFrame = CFrame.new(target.Character.HumanoidRootPart.Position + Vector3.new(0, 9, 0), target.Character.HumanoidRootPart.Position) * CFrame.Angles(0, tick() * 30, 0)
...
UserInputService.InputBegan:Connect(function(i, gp)
    if not gp and i.KeyCode == Enum.KeyCode.RightShift then
        ...
        if MyUI then MyUI.Enabled = not MyUI.Enabled end
    end
end)
